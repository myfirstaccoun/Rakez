...
function run() {
    ...
    let videoShorts = document.querySelector("#shorts-inner-container");
    ...
    if(videoShorts != null) {
        videoShorts.querySelector("dom-repeat.style-scope.ytd-shorts") != null? videoShorts.querySelector("dom-repeat.style-scope.ytd-shorts").remove() : "";
        
        videoShorts.querySelectorAll("ytd-reel-video-renderer.reel-video-in-sequence.style-scope.ytd-shorts").forEach((video) => {
            if(JSON.parseInt(video.id) > 1) {
                video != null? video.remove() : "";
            }
        })
    }
